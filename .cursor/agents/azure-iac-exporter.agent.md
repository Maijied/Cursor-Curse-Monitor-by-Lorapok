...
- **IaC Code Generation**: Use #tool:agent/runSubagent to call azure-iac-generator with comprehensive infrastructure requirements for target format generation with format-specific validation
...
- Use `#tool:execute` with `az rest` commands to collect only user-configured data plane properties:
  ...
  - **App Service**: `az rest --method POST --url "https://management.azure.com/{appServiceId}/config/appsettings/list?api-version=2023-01-01"` → Extract custom application settings only
...
- Use #tool:agent/runSubagent to call azure-iac-generator with filtered resource analysis (user-configured properties only) and infrastructure requirements for format-specific template generation
...
- Include resource-specific data plane configurations when relevant
...
- **Main IaC Template**: Primary resource definition in chosen format
...
2. **Smart Resource Discovery**: "Please provide the Azure resource name (e.g., 'myvm', 'mywebapp'). I'll automatically find it across your subscriptions."
...
4. **Disambiguation (if needed)**: If multiple resources found:
   ...
   ```
   Found multiple resources named 'myvm':
   1. myvm (Resource Group: rg-prod-eastus, Type: Virtual Machine, Location: East US)
   2. myvm (Resource Group: rg-dev-westus, Type: Virtual Machine, Location: West US)

   Please select which resource to export (1-2):
   ```

5. **Azure Resource Graph (Control Plane Metadata)**: Use `#tool:ms-azuretools.vscode-azure-github-copilot/azure_query_azure_resource_graph` to get comprehensive resource properties and control plane metadata
6. **Azure MCP Resource Tool Call (Data Plane Metadata)**: Call appropriate Azure MCP tool based on resource type:
   - For Storage Account: Call `#tool:azure-mcp/storage` to gather data plane metadata
   - For Key Vault: Call `#tool:azure-mcp/keyvault` for vault data plane metadata
   - For AKS: Call `#tool:azure-mcp/aks` for cluster data plane metadata
   - For App Service: Call `#tool:azure-mcp/appservice` for application data plane metadata
...
10. **IaC Code Generation**: Call azure-iac-generator subagent with analysis summary and infrastructure requirements:
    ...
    - Call #tool:agent/runSubagent with `agentName="azure-iac-generator"` providing:
