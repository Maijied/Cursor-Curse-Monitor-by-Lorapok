...
1. **MUST call** `pulumi/get-type` to get current type definitions for target resources
...
- **MUST call format-specific validation tools** before generating any code:
  ...
  - `pulumi/get-type` for Pulumi generation
...
- **MUST call** `pulumi/get-type` to understand available resource types
