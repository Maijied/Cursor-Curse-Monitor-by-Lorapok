...
Classify every selected or generated local asset with the purpose rules in [superdesign.md](superdesign.md). A visual reference uses `--purpose reference`; a logo or reusable identity asset uses `--purpose brand`; imagery that must render in the deck uses `--purpose content`. Upload content imagery like this:

```bash
npx --yes @superdesign/cli@latest upload-asset "/absolute/path/to/asset.png" \
  --project-id <id> --purpose content
```

Keep default canvas placement. Record each returned URL and reference identifier together with a short visual description and its intended slide role. Use the URL when the slide must render the image and `--reference-id` whenever Superdesign must inspect its pixels. Reuse assets where continuity helps, but do not force the same background onto every slide. If a supplied or existing logo is required, use its exact Brand Asset URL and asset key rather than recreating it. Render charts, diagrams, labels, and exact data in the editable slide layer rather than baking them into images.
...
Correct exact copy, source, URL, image, viewport, or overflow defects with the local HTML edit/import route in [superdesign.md](superdesign.md). Use `--mode replace` only when a slide needs creative visual recomposition; do not branch a correction into another slide. Tell the user clearly that the canvas contains slide designs and that native PowerPoint export is outside this version unless another available tool is explicitly used afterward.
