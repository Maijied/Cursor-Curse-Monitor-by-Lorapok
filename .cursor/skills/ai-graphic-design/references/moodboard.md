...
Accept locally available user uploads, AI-generated images, screenshots, brand assets, and visual references. When the current session generates images, upload the resulting local files when the user asks to save or collect them. Classify each image by its intended role: inspiration and screenshots are `reference`, reusable identity files are `brand`, and images intended for final artifacts are `content`.
...
Replace `reference` with the correct purpose when the image is brand or content material; include the required brand type, key, and description for a brand upload. Keep the default canvas placement. Never pass `--no-canvas` in this workflow. Record the returned public URL and identifier with the source filename or user description so the asset can be reused later.
...
npx --yes @superdesign/cli@latest create-design-draft \
  ...
  --reference-id <selected-node-id-or-asset-key> \
  --user-request "<verbatim user request>"
```

Pass every selected image whose pixels must guide the exhibition after the same variadic flag: `--reference-id <id-1> <id-2>`. Only add sections, captions, filtering, navigation, motion, or other behavior the user's goal supports. Keep the uploaded source assets on the project canvas, and share the resulting preview together with the project canvas.
