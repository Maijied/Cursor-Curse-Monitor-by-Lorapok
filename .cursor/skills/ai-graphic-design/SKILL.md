...
- Classify every supplied image before upload: temporary visual inspiration is a `reference`, reusable identity is a `brand` asset, and imagery that must render in the finished artifact is `content`.
- A URL in prompt text does not guarantee that the design model can see the image. Pass the returned reference identifier to generation whenever the model must inspect its pixels; also include the returned public URL when the finished HTML must render that exact image.
- Reuse supplied and existing project assets before generating replacements. If an exact logo is available, use that logo wherever the artifact calls for one; never redraw, approximate, or substitute it.
...
- Route revisions by intent: directly edit exact factual or literal defects, replace an accepted draft for creative refinement, and branch only for a genuinely separate option or artifact.
