...
description: >-
  Lorapok Labs brand and frontend polish for Cursor Curse Monitor. Use when
  changing website, admin SPA, IDE dashboard, or browser-extension UI. Preserve
  the restored dark mission-control look; do not replace it with generic AI
  purple-on-white or Inter/Roboto defaults.
---
...
**Cursor Curse Monitor by Lorapok** — a live usage dashboard for Cursor and other VS Code–wrapper AI IDEs. Audience: developers who need quota, billing cycle, and fallback status at a glance. The page's job is to get them to install, then trust that data stays local.
