...
export async function importConversationIndexPackage(
  ...
  if (!policy.cipAllowCrossUserLocalImport) {
    if (!packageOwnerHash) {
      return {
        success: false,
        error:
          "Package is missing ownerHash. Cross-account import is disabled by Mission Control policy.",
        imported: 0,
        skipped: 0,
        searchIndexed: [],
        sidebarRestored: [],
    ...
    if (packageOwnerHash !== localOwnerHash) {
      return {
        success: false,
        error:
