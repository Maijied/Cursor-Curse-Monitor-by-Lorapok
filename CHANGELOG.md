...
- **Subscribe KV quota resilience** — upsert looks up one scatter key first (no snapshot read on resubscribe); skip snapshot writes when stats refresh is paused; subscriber list falls back to capped scatter rebuild when snapshot read is quota-blocked; **D1 fallback** stores new subscribers in `subscriber_index` when KV writes are exhausted
