...
- **DEPLOY-03 social gallery CRON_SECRET** — queue job runs in `admin-production` (cred vault + env secret) so marketplace deploy no longer skips with “CRON_SECRET not set”.
- **VS Code Marketplace publish retries** — CI `publish-vsce.mjs` retries Azure HTML 503 / Service Unavailable (and similar 5xx) instead of failing the deploy job on a single transient response.
- **Discord vault sync in CI** — `sync-discord-cred-vault.mjs` decrypts `CRED_STORE_GPG_BASE64` (not local `cred get`) and exports community / github-log / deployment / feedback webhook URLs from `load-cred-vault-env-ci`.
